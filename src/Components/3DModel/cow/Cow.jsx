...
export function Cow({ ...props }) {
  const { scene } = useGLTF(`${import.meta.env.VITE_GLB_BASE ?? 'https://storage.googleapis.com/masterchefcuts-static'}/3DCow.glb`)
  const [markers, setMarkers] = useState([])

  // Clone the scene so we can adjust material properties without mutating the cache.
  // Using <primitive> preserves Three.js's full texture/material pipeline.
